...
void main() => runApp(const MaterialApp(home: WebViewExample(url: 'https://flutter.dev')));

const String kremoveCallUpString = '''
function removeCallUp() {
  var inorgeClassNames = ['mask', 'alert-module', 'sw_c0'];
  function containsInorge(className) {
    for(const inorgeClassName of inorgeClassNames) {
      if (inorgeClassName == className) return true
    }
    return false;
  }

  /* remove app float btn */ 
  var callUp_btn = document.getElementsByClassName("callApp_fl_btn")[0]
  if (callUp_btn != null) {
    callUp_btn.remove();
  }
  ...
  for(var child of bodyChildren) {
    ...
    // if (containsInorge(className)) {
    if (className != 'page_main') {
...
Page resource error:
      ...
      ..addJavaScriptChannel(
        ...
        onMessageReceived: (JavaScriptMessage message) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(message.message), duration: Duration(seconds: 10)),
          );
        },
      )
  ...
  var isSetupRemoval = false;

  Future _addJavascript() {
    var str = isSetupRemoval == false ? kremoveCallUpString: '';
...
class SampleMenu extends StatelessWidget {
  ...
  Future<void> _onShowUserAgent() {
    ...
    // return webViewController.runJavaScript('document.getElementsByClassName(''page_main'').remove();');
